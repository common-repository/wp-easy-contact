...
function emd_lite_get_operations($type,$plural_label,$textdomain){
	...
	if($type == 'opr'){
		echo '<h2>' . sprintf(__('Import, Export and Update %s From/To CSV', 'emd-plugins'),esc_attr($plural_label)) . '</h2>
		<div style="max-width:450px;margin: auto;"><strong>' . sprintf(__('Do you have hundreds of %s in a spreadsheet and think about how to get them in WordPress? No problem!', 'emd-plugins'),esc_attr($plural_label)) . '</strong></div>
		<div style="max-width:450px;margin: auto;padding-bottom:15px;">' . 
	sprintf(__('Sometimes, adding or updating %s one by one is NOT the smartest thing to do. Bulk import, update and take snapshots at will - more time to play!', 'emd-plugins'),esc_attr($plural_label)) .
		'</div>
		<div class="emd-flex emdflexrow">
		<ul class="left">
		<li><span class="emdiconpad dashicons dashicons-thumbs-up" aria-hidden="true"></span>' . sprintf(__('Get your %s in %s fast', 'emd-plugins'),esc_attr(strtolower($plural_label)),esc_attr($plugin_name)) . '</li>
		<li><span class="emdiconpad dashicons dashicons-thumbs-up" aria-hidden="true"></span>' . sprintf(__('Bulk update %s with a single click', 'emd-plugins'),esc_attr(strtolower($plural_label))) . '</li>
		...
		<li><span class="emdiconpad dashicons dashicons-thumbs-up" aria-hidden="true"></span>' . sprintf(__('Migrate %s from one site to another with ease', 'emd-plugins'),esc_attr(strtolower($plural_label)))  . '</li>
	...
	elseif($type == 'yt_api'){
		echo '<h2>' . sprintf(__('Get your YouTube videos and update stats with a few clicks', 'emd-plugins'),esc_attr($plural_label)) . '</h2>
		<div style="max-width:450px;margin: auto;"><strong>' . sprintf(__('You can get your YouTube videos in WordPress and update their stats on demand or schedule. Bonus: Video updates are done automatically. ', 'emd-plugins'),esc_attr($plural_label)) . '</strong></div>
		<div style="max-width:450px;margin: auto;padding-bottom:15px;">' . 
	sprintf(__('You can add multiple playlists and/or usernames and decide how often you want to get new videos or make updates to existing ones. Easy!', 'emd-plugins'),esc_attr($plural_label)) .
		'</div>
		<div class="emd-flex emdflexrow">
		<ul class="left">
		<li><span class="emdiconpad dashicons dashicons-thumbs-up" aria-hidden="true"></span>' . sprintf(__('Import from unlimited playlists', 'emd-plugins'),esc_attr(strtolower($plural_label)),esc_attr($plugin_name)) . '</li>
		<li><span class="emdiconpad dashicons dashicons-thumbs-up" aria-hidden="true"></span>' . sprintf(__('Import from unlimited usernames', 'emd-plugins'),esc_attr(strtolower($plural_label))) . '</li>
		...
		<li><span class="emdiconpad dashicons dashicons-thumbs-up" aria-hidden="true"></span>' . sprintf(__('On-demand video stat updates', 'emd-plugins'),esc_attr(strtolower($plural_label)))  . '</li>
	...
	elseif($type == 'cust_fields'){
		...
		<div class="emd-oper-modal-button">
			<a href="' . esc_url("https://emdplugins.com/plugin-pricing/wp-easy-contact-wordpress-plugin-pricing/?pk_campaign=" . esc_attr($app) . "-cust-fields&pk_kwd=upgradelink") . '" class="emdmodalbtn" target="_blank" rel="noopener noreferrer">';
...
function emd_lite_get_filters($textdomain){
	...
	echo '<div id="emd-afc-filters" class="metabox-holder meta-box-sortables">
		...
		<div class="handlediv upgrade-pro" title="' . __('Update to a Premium Edition to unlock Filters & Columns', esc_attr($textdomain)) . '"></div>
		<h3 class="upgrade-pro" title="' . __('Update to a Premium Edition to unlock Filters & Columns', esc_attr($textdomain)) . '">' . __('Filters &amp; Columns', esc_attr($textdomain) ) . '</h3>
	</div></div>';
}
