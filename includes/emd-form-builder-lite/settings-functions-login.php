...
?>
	...
	<?php	echo '<form method="post" action="options.php">';
		...
		echo '</select>';
		echo "<p class='description'>" . sprintf(__('Use [emd_login app="%s"] shortcode in your page. Attributes: ent="emd_customer,emd_vendor" show_reg=1 , reg_label="",reg_link="",show_lost=1  ','emd-plugins'),esc_html($app)) . "</p></td></tr>";
		...
		if(!empty($cust_roles)){
			...
			foreach($cust_roles as $krole => $vrole){
				...
				echo sprintf(__('%s Login','emd-plugins'),esc_html($vrole));
				echo '</label></th><td>';
				...
				echo sprintf(__('%s Logout','emd-plugins'),esc_html($vrole));
				echo '</label></th><td>';
