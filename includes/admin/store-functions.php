...
if (!function_exists('emd_display_store')) {
	function emd_display_store($textdomain) {
		...
		ob_start(); ?>
	<div class="wrap">
	<h2><?php echo esc_html($title);?> &nbsp;&mdash;&nbsp;<a href="https://emdplugins.com/plugins?pk_source=plugin-addons-page&pk_medium=plugin&pk_campaign=<?php echo esc_attr($textdomain);?>-addonspage&pk_content=browseall" class="button-primary" title="<?php _e( 'Browse All', 'emd-plugins' ); ?>" target="_blank"><?php _e( 'Browse All', 'emd-plugins' ); ?></a>
	</h2>
	<p><?php _e('The following plugins extend and expand the functionality of your app.','emd-plugins'); ?></p>
			<?php echo esc_html(emd_add_ons('tabs',$textdomain)); ?>
		</div>
		<?php
		echo ob_get_clean();
	}
...
if (!function_exists('emd_display_support')) {
	...
	<?php 
		...
		echo '</div>';
		echo esc_html(emd_add_ons('plugin-support',$textdomain)); 
		echo '</div>';
		echo ob_get_clean();
	}
}
if (!function_exists('emd_display_review')) {
	...
	<div class="plugin-review-text"><a href="https://wordpress.org/support/view/plugin-reviews/<?php echo esc_attr($plugin); ?>" target="_blank"><?php _e('Like our plugin? Leave us a review','emd-plugins'); ?></a>
