...
class Emd_Single_Taxonomy {
	...
	?>
			<div id="taxonomy-<?php echo esc_attr($taxonomy); ?>" class="radio-buttons-for-taxonomies">
			...
			<li class="hide-if-no-js"><a href="#<?php echo esc_attr($taxonomy); ?>-pop" tabindex="3"><?php _e('Most Used', 'emd-plugins'); ?></a></li>
			</ul>
			<style>
			...
			<div id="<?php echo esc_attr($taxonomy); ?>-pop" class="wp-tab-panel tabs-panel" style="display: none;">
			<ul id="<?php echo esc_attr($taxonomy); ?>checklist-pop" class="<?php if (is_taxonomy_hierarchical($taxonomy)) {
		...
		<?php $popular = get_terms($taxonomy, array(
			...
			<?php foreach ($popular as $term) {
				...
				echo "<input type='radio' id='in-" . esc_attr($id) . "'" . checked($current_id, $term->term_id, false) . " value='" . esc_attr($value) . "' " . esc_attr($disabled) . " />&nbsp;" . esc_html($term->name) . "<br />";
				echo "</label></li>";
			} ?>
		</ul>
			</div>
			<div id="<?php echo esc_attr($taxonomy); ?>-all" class="wp-tab-panel tabs-panel">
			<?php
			$name = 'radio_tax_input[' . esc_attr($taxonomy) . ']';
			echo "<input type='hidden' name='" . esc_attr($name) . "[]' value='-1' />"; // Allows for an empty term set to be sent. 0 is an invalid Term ID and will be ignored by empty() checks. -1 is used instead of 0
			
	?>
			<ul id="<?php echo esc_attr($taxonomy); ?>checklist" data-wp-lists="list:<?php echo esc_attr($taxonomy); ?>" class="<?php if (is_taxonomy_hierarchical($taxonomy)) {
				echo 'categorychecklist';
			} else {
				echo 'tagchecklist';
			} ?> form-no-clear">
	...
	?>
		...
		<input type="button" id="<?php echo esc_attr($taxonomy); ?>-add-submit" data-wp-lists="add:<?php echo esc_attr($taxonomy); ?>checklist:<?php echo esc_attr($taxonomy); ?>-add" class="button <?php if (is_taxonomy_hierarchical($taxonomy)) {
		...
		if (!empty($this->single_tax_list)) {
			foreach ($this->single_tax_list as $stax => $sval) {
				if (isset($_POST['post_type']) && in_array($_POST['post_type'],$sval['ptype'])) {
					...
					if (isset($_POST["radio_tax_input"]["{$stax}"])) $terms = array_map('sanitize_text_field',$_POST["radio_tax_input"]["{$stax}"]);
